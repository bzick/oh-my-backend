...
* Установка пакетов и обновление системы через `apt`/`apt-get`/`aptitude`. > без пакетов - никуда
* Базовые навыки в bash.
  > в linux-подобных системах башом пронизано всё, вы гарантировано столкнётесь с ним при работе.
  ...
  * Конвееры команд через оператор `|`: `cmd1 | cmd2 | cmd3`.
    > В unix системах все операции разбиты на отдельные команды и для решения различных задач придётся их объединять через коневееры.
  ...
  * Консольные редакторы `vim`, `nano`. Откыть файл, внести изменения, сохранить. 
    > Редактирование файла из консоли не такая редкость. Кстати, что бы выйти из vim: esc, `:q!`, enter.
  * Консольные просмоторщики `less`, `zless`. Открыть, найти слово, закрыть.
    > Иногда редакторы "слишком мощные" что бы посмотреть содержимое файла. Просмоторщик так же справляется с не "стандартными" файлами.
...
* Понятие `процесс`
  * Команды анализа процессов `top`/`htop`, `ps` (`ps aux`).
    > "Диспетчер задач" мира unix систем.
  * Родительский процесс, дочерний процесс.
    > Процессы не появляются из ниоткуда, их кто-то запускает. Понимание иерархии процессов.
  * Мастер-воркер процессы, демон (daemon)
    > Это один из распространённых видов распараллеливания вычислений в проиложениях. Много программ в unix системах используют такой подход.
  * Зомби процесс. Откуда берутся, как с ними бороться. 
    > Это вид проблемы распаралелливания вычислений через доченирние процессы. Возникает когда у мастер-процесса есть проблемы или баги.
