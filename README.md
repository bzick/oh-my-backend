...
Документ содержит список базовых навыков, которые **часто** требуются backend разработчику web-приложений. Документ еще находится в процессе дополнения.
...
Как это работает "внутри" (принцип работы) будет дополнительным бонусом в понимании темы. 
Если разбираться как работает "внутри" каждый пункт, то потребуется очень много времени на изучение, и как следствие, изучайте по желанию и необходимости. Это не квантовая физика, глубокое понимание одних технологий упрощает понимание других технологий.

Каждый пункт легко гуглится и имеет страницу в wikipedia. 
...
Тут они используются что бы помочь разделить кучу тем на то что стоит изучать в первую очередь. 
Тут применяется [стандартная градация](https://vas3k.ru/blog/team/) навыков и зон ответственности, поэтому <kbd>senior</kbd> это <kbd>middle+</kbd> с soft-скилами.
...
Изучить установленный в контейнере Linux. Linux де-факто является серверной ОС для большинства случаев web-приложений.
...
1. Базовые навыки в `bash` (улучшенный `sh` aka `shell`). <kbd>junior</kbd>
   ...
   * Базовый синтаксис bash. <kbd>middle</kbd>
     ...
     * Управляющие операторы `for` и `if`
     * Логические `;`, `&&`, `||`
...
1. Агрегации <kbd>middle</kbd> <kbd>middle+</kbd>
...
По факту сейчас среди opensource актуален [Prometheus](https://prometheus.io/) (или подобные, например, [Victoria Metrics](https://victoriametrics.com/)) для сбора
...
1. [Шаблоны проектирования](https://refactoring.guru/ru/design-patterns/examples)
   > Шаблоны упрощают разработку так как это, по сути, опыт сообщества по решению тех или иных проблем. Главное не забывайте про KISS и YAGNI что бы не упасть в ад абстракций и не сделать всё сложнее.
...
1. Распределенные системы
   * Теорема CAP
     ...
     > Теорема не совершенна и имеет противоречия и как следствие её расширили до теоремы PACELC.
   * Теорема PACELC
     > Расширенная теорема CAP, где уже больше параметров распределённой системы которые вы можете реализовать,
     > принеся несколько других параметров распределённой системы в жертву.
