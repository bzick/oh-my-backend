...
А где <kbd>sinior</kbd>? Что еще за <kbd>high middle</kbd>?
Sinior смотрит на это с грустью в глазах, [так как это уже про софт-скиллы](https://vas3k.ru/blog/team/). А <kbd>high middle</kbd> как раз путают с <kbd>sinior</kbd>.
У все свои разделения на градации, тут они используются что бы помочь поделить кучу знаний на то что стоит изучать в первую очередь.
...
* [Этап 10. Полнотексовый поиск через ElasticSearch](#этап-10-полнотексовый-поиск)
...
1. Перенос контента между хостами. <kbd>junior</kbd>
    ...
    * `scp` — самый простой и "нативный" инструмент переноса файлов между хостами по ssh. <kbd>junior</kbd>
    * `rsync` — пожалуй самы мощный инструмент переноса файлов между хостами. <kbd>middle</kbd>
1. Планировщик задач `crontab`, `at` <kbd>middle</kbd>
...
1. SQL базы данных MySQL/Postgres/итд. MySQL подразумевает как MySQL от Oracle, так и различные модификации как MariaDB, Percona XTraDB. 🟢
   ...
   * [Понимание работы индексов](https://ruhighload.com/Индексы+в+mysql) <kbd>junior</kbd>
