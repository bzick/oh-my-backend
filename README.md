...
Даже если вы уже имеете Linux на рабочей машине, всё равно поднимите виртуализацию. В случае чего, виртуальную машину всегда можно пересоздать.
Есть много систем виртуализации, но docker выделяется среди них. Docker — популярный инструмент для десктопной виртуализации. На серверах к нему прибегают в меньшей степени, так как там более популярен Kubernetes (aka k8s). Docker не единственная система виртуализации, ближайшие аналоги это [Lima](https://github.com/lima-vm/lima), [Podman](https://podman.io/) (нечто среднее между `docker` и `k8s`).
...
1. docker compose для поднятия кластера контейнеров. <kbd>middle</kbd>
   ...
   > и всё это упаковать в один docker контейнер будет проблемно, просто из-за специфики работы самого docker'а.
...
1. Базовое понимание работы сети. <kbd>junior</kbd>
   * Протокол TCP <kbd>middle</kbd>
     ...
     * TCP пакет <kbd>guru ⚡</kbd>
       > Врят-ли придётся работать с пакетом TCP напрямую, однако из его стуктуры можно подчерпнуть некоторую полезную информацию по протоклоу TCP вцелом.
     * Флаги ACK, SYN, FIN и прочие <kbd>middle+</kbd>
       > Флаги отвеают за организацию, поддержание и закрытие TCP коннектов. 
       > И их работа и скорость передачи напорямую влияет на скорость передачу данных 
     * Буферы (window size) <kbd>middle</kbd>
     * [Проблемы TCP](https://www.youtube.com/watch?v=aXYJlizk3CQ) <kbd>middle+</kbd>
       > TCP очень старый протокол, который не удовлетворяет современным реалиям.
   ...
   * Протокол UDP. <kbd>middle</kbd>
     ...
     * UDP-пакет <kbd>guru ⚡</kbd>
       > Изучить придётся только если придётся создавать свой протокол передачи данных взамен изжившего себя TCP. 
...
1. SQL базы данных MySQL/Postgres/итд. <kbd>junior</kbd>
   ...
   > В общем понимании семейства: MySQL/Postgres/MSSQL/и т.д. имеют схожие SQL API, различаются только внутренней реализацией, производительностью и масштабируемостью. В данной теме будет упор на MySQL семейство, которое очень распространено.
   * Базовый синтаксис запросов `SELECT`/`INSERT`/`UPDATE`/`DELETE`. <kbd>junior</kbd>
   * Создание и модификация таблиц <kbd>junior</kbd>
       * Типы колонок таблиц их назначение и различие (на примере MySQL и схожих) <kbd>junior</kbd>
...
1. Ваш язык программирования <kbd>junior</kbd>
   ...
   * Объекты/классы/структуры, прототипы/интерфейсы/миксины.
     > Все современные языки имеют объекты и умеют ими оперировать. Отличаются только возможности манипуляции надо объекто или самим объектом.
     * Перегрузка операторов
       > Позволяет объектам самим определять как над ними производить математические, логически и прочие операторы.
     * Перегрузка методов
       > Позволяет определить несколько методов с разной сигнатурой (обычно с разным набором аргументов).
     * Generics (aka генерики aka шаблоны aka обобщения)
       ...
       > Позволяет определить некоторую одну функциональность (метод или методы) для нескольких типов.
     * Объектно-ориентированное программирование (ООП)
       > Распространённый подход в популярных язках программирования, подразумевающий что всё — объекты и надо оперировать ими.
     * Интерфейсы 
       > Описание сигнатуры выших объектов (и только). Встречается не только в языках с развитым ООП.
   ...
   * Слабая/сильная типизация в коде. На что влияет и как с этим жить.
     ...
     > А динамическая, наоборот, позволяет не указывать тип. Вычисление типа значения будет происходить динамически, прямо на ходу. 
...
1. Архитектурные шаблоны <kbd>junior</kbd>
   > В одном проекте может быть один или несколько архитектурных шаблонов, или даже половина. Архитектурные шаблоны - не архитектура вашего приложения. 
   > Это подход к решению задачи, которую возложили на проект. Заумно вышло...
...
1. Тестирование. <kbd>junior</kbd>
   ...
   * End-to-End (aka E2E aka Сквозное тестирование) <kbd>middle</kbd>
     > Пример E2E теста - тестирование готового HTTP API приложения. Тестируются не компоненты приложения, а готовая функциональность.
