...
**Что это?** Этот документ содержит список базовых навыков, которые **часто** требуются backend разработчику web-приложений. Документ разделён на этапы (темы), каждый этап разделён на пункты. Каждый пункт, в документе, подразумевает что:
...
Принцип работы пункта будет дополнительным бонусом в понимании темы, но может это может занять много времени. Изучайте по желанию и необходимости.
...
В разделе будет говориться о linux, хотя _почти_ всё так же актуально и для других posix-совместимых систем таких как [bsd семейство](https://ru.wikipedia.org/wiki/BSD) (включая macos), но могут быть отличия. 
...
Работа с эл. письмами неотъемлемая часть web разработки (да и не только). По факту, это едиственный гарантированный канал связи с пользователем.
