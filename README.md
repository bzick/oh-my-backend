...
Изучить установленый в контейнере Linux из этапа 1. Linux де-факто является серверной ос для большинства случаев web-приложений.
  ...
  * Оперативная память
    ...
    * Ошибка `Out Of Memory` (OOM) и причина появления.
  * Логи. Для чего и как посмотреть.
    * `dmesg` (driver messages) — выхжные сообщения от компонентов linux, включая об OOM.
...
Любое приложение должно уметь генерировать _полезные_ метрики для система сбора и анализа метрик. По факту сейчас среди opensource акуален [Prometheus](https://prometheus.io/) (или подобные) для сбора и анализа метрик и [Grafana](https://grafana.com/) для их отображения и настройки алертов.
